...
import { useServices } from '@/services/adminServices/services/servicesServices';
...
export const ModalAddService = ({ isOpen, onClose, saveInfo }: ModalAddServiceProps) => {
    ...
    useEffect(() => {
        if (selectedService != null){
            const filteredService = services.filter(s => Number(s.id_company_service) == Number(selectedService))[0]
    ...
    const handleSubmit = () => {
        ...
        const selectService = services.filter(s => Number(s.id_company_service) == Number(selectedService))[0]
        const selectCategorie = selectCategorias.filter(s => Number(s.idCategoria) == Number(selectedCategorie))[0]

        // Gera um ID aleatório simples
        const randomId = `${Date.now()}-${Math.floor(Math.random()*10000)}`;
        saveInfo({
            randomId: randomId,
            id_categoria: selectCategorie.idCategoria,
            categoria: selectCategorie.nome,
            id_service: selectService.id_company_service,
            ...
            >
                <div className="w-full p-2 pt-4 flex flex-col justify-center items-center gap-4 ">
                    ...
                    <SelectPicker
                        ...
                        data={selectCategorias.map(c => { return { value: c.idCategoria, label: c.nome } })}
                        // searchable={false}
                        style={{ width: 216 }}
                    ...
                    <SelectPicker
                        ...
                        placeholder='Serviço'
                        data={services.map(c => { return { value: c.id_company_service, label: c.service } })}
                        // searchable={false}
                        style={{ width: 216 }}
